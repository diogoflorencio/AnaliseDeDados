...
```{r}
highchart() %>% 
  hc_xAxis(TWD$season_ep) %>%
  hc_add_series(name = "temporada 1" , data = (TWD %>% filter(season == 1))$UserRating) %>%  
  hc_add_series(name = "temporada 2" , data = (TWD %>% filter(season == 2))$UserRating) %>%  
  hc_add_series(name = "temporada 3" , data = (TWD %>% filter(season == 3))$UserRating) %>%
  hc_add_series(name = "temporada 4" , data = (TWD %>% filter(season == 4))$UserRating) %>%  
  hc_add_series(name = "temporada 5" , data = (TWD %>% filter(season == 5))$UserRating) %>%  
  hc_add_series(name = "temporada 6" , data = (TWD %>% filter(season == 6))$UserRating) %>%
  hc_add_series(name = "temporada 7" , data = (TWD %>% filter(season == 7))$UserRating)
```
...
```{r}
hcboxplot(x = dados$UserRating, var = dados$series_name) 
```
