...
Para responder essa pergunta estimei a estatística *p^ = media[media(2016)/media(2017)]*, que seria a razão entre os intervalos estimados para popularidade de cada linguagem para 2016 e 2017.
```{r}
...
```
A análise também foi realizada com confiança de 95%, o bootstrap gerou 10.000 reamostragens. Os intervalos estimados para `p^` foram [1.169852 ; 1.171908] para `java` e [1.10705 ; 1.109022] para `phyton`. Ou seja, pode-se dizer que a popularidade de java caiu de 1.69 à 1.71 vezes entre 2016 e 2017. No caso de python sua popularidade caiu em 1.1 vezes, entre os mesmos anos. Este resultado é relevante pois apresenta a proporção da queda de popularidade para as duas linguagens de interesse. 
...
cor_pearson = bootstrap(popularidade_java_python, cor(media_java, media_py , method = "pearson")) %>% 
                        CI.percentile(probs = c(.025, .975)) %>% 
                        as.data.frame() %>%
                        mutate(corr = "pearson")

cor_kendall = bootstrap(popularidade_java_python, cor(media_java, media_py , method = "kendall")) %>% 
                        CI.percentile(probs = c(.025, .975)) %>% 
                        as.data.frame() %>%
                        mutate(corr = "kendall")

cor_spearman = bootstrap(popularidade_java_python, cor(media_java, media_py , method = "spearman")) %>% 
                        CI.percentile(probs = c(.025, .975)) %>% 
                        as.data.frame() %>%
                        mutate(corr = "spearman")

#plotando correlação entre a popularidade de java e python 
data.frame(rbind(cor_pearson, cor_kendall, cor_spearman)) %>% 
